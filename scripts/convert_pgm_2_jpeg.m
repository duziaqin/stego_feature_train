function  conver_pgm_2_jpeg(conf, imageSeriersFromCommad)
	...
	[type, imageTypes, imageSeriers, bpps, algorithms, BASE_PATH, IMAGES_PATH, ...
	...
	for single = startPoint:endPoint
		coverPath = fullfile(BASE_PATH, 'images', 'cover', [generatePicName(single), '.', IMAGE_PREFIX]);
		saveCoverPath = fullfile(IMAGES_PATH, 'cover', [generatePicName(single), '.jpeg']);
			...
			for bppIndex = 1:bpps_length
				...
				imagePath = fullfile(BASE_PATH, 'images', 'stego', imageType, bpp, [generatePicName(single), '.', IMAGE_PREFIX]);
				savePath = fullfile(IMAGES_PATH, 'stego', imageType, bpp, [generatePicName(single), '.jpeg']);
